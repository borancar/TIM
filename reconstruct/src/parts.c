...
void part_setup_065b(uint16_t part)
{
    ...
    if (PART(part).flags_08 & 0x10)
        tab = PART(part).form == 0 ? 0x320a : 0x3216;
    else
        tab = PART(part).form == 0 ? 0x31f2 : 0x31fe;
...
{
    uint16_t tab = PART(part).form == 0 ? 0x3274 : 0x3282;
 ...
 */
void part_setup_1105(uint16_t part)
{
    uint8_t a, b = 0, c;
    uint16_t di;

    if (PART(part).kind == 0x37) {
        a = 0x54;
    } else if (PART(part).kind == 0x39
               && PART(part).form == 8) {
        a = 0x69;
        b = 0x0a;
    } else {
        a = (uint8_t)(PART(part).width - 1);
    }

    if (PART(part).kind == 0x39
        && PART(part).form == 0)
        c = 1;
    else
    ...
    PART(part).byte_6a =
        (uint8_t)(((int16_t)PART(part).width) >> 1);
    PART(part).byte_6b = 0;
}
 ...
 */
void part_setup_1435(uint16_t part)
{
    int32_t on = (PART(part).flags_08 & 0x10) != 0;
    uint16_t tab = on ? 0x32ae : 0x32a4;
    uint16_t si;
    int32_t k;

    PART(part).grab_x = on ? 0x25 : 0x00;
    PART(part).grab_y = 0x0d;
    PART(part).word_58 = 0x12;
...
void part_setup_1556(uint16_t part)
{
     ...
     */
    uint16_t tab = ((int16_t)PART(part).form) < 4 ? 0x32b8 : 0x32c0;
    ...
    part_finish(0x5d1e, part);
    PART(part).point_count = 1;

    PART(part).form =
        (uint16_t)(PART(part).form & 4);
    if (PART(part).linked_a != 0)
        PART(part).form =
            (uint16_t)(PART(part).form | 1);
    if (PART(part).linked_b != 0)
        PART(part).form =
            (uint16_t)(PART(part).form | 2);
}
 ...
 */
void part_setup_2068(uint16_t part)
{
    uint16_t di;
    int32_t i;
    ...
    for (di = DG521B.parts_ptr; di != 0; di = ((uint16_t)PART(di).link_ptr)) {
        ...
        dx = (int16_t)(((int16_t)PART(part).word_8c)
                       - ((int16_t)PART(di).word_8c));
        dy = (int16_t)(((int16_t)PART(part).word_8e)
                       - ((int16_t)PART(di).word_8e));

        if (dy == 0) {
            if (dx == 0x20)
                PART(part).link_right = di;
            else if (dx == -0x20)
                PART(part).link_left = di;
        } else if (dx == 0) {
            if (dy == 0x20)
                PART(part).link_down = di;
            else if (dy == -0x20)
                PART(part).link_up = di;
        }
    }
}
...
void part_setup(uint16_t off, uint16_t part)
{
     ...
     */
    {
        ...
        for (j = 0; j < 4; j++) {
            ...
            {
                uint16_t set = (PART(part).flags_08 & 0x10) != 0;
     ...
     */
    {
        ...
        for (j = 0; j < 4; j++) {
            ...
            tab = (PART(part).flags_08 & 0x10) ? two[j].set
                                                       : two[j].clear;
            si = PART(part).points_ptr;
            for (k = 0; k < two[j].n; k++) {
                DG8((uint16_t)(si + 4 * k)) = DG8((uint16_t)(tab + 2 * k));
...
/*
     ...
     */
if (off == 0x23b1) {
        part_setup_23b1(part);
        return;
    }

    if (off == 0x1435) {
        part_setup_1435(part);
        return;
    }

    if (off == 0x2068) {
        part_setup_2068(part);
        return;
    }
...
if (off == 0x3294) {
        part_setup_3294(part);
        return;
    }

    if (off == 0x0b88) {
        part_setup_0b88(part);
        return;
    }

    if (off == 0x1261) {
        part_setup_1261(part);
        return;
    }

    if (off == 0x19db) {
        part_setup_19db(part);
        return;
    }

    if (off == 0x2cce) {
        part_setup_2cce(part);
        return;
    }
...
if (off == 0x377b) {
     ...
     */
    {
        ...
        for (j = 0; j < 2; j++) {
            ...
            on = (PART(part).flags_08 & 0x10) != 0;
            PART(part).byte_72 = on ? sized[j].set72
                                              : sized[j].clear72;
            PART(part).byte_73 = sized[j].b73;
 ...
 * bit `part_flip_19fa` turns over, where every other flip in this segment uses
 * bit 4. It does not call `part_finish_angles`, because it changed nothing
 * that would need the angles redone.
 */
void part_setup_19db(uint16_t part)
{
    PART(part).byte_6a = 7;

    if (PART(part).flags_08 & 0x20)
        PART(part).byte_6b = 0x0e;
    else
        PART(part).byte_6b = 1;
}
...
{
    uint16_t si = (PART(part).flags_08 & 0x10) ? 0x322a : 0x3222;
    uint16_t di = PART(part).points_ptr;
    int16_t i;
...
{
    uint16_t si = (PART(part).flags_08 & 0x10) ? 0x325c : 0x3252;
    uint16_t di = PART(part).points_ptr;
    int16_t i;
...
{
    uint16_t si = (PART(part).flags_08 & 0x10) ? 0x32d2 : 0x32c8;
    uint16_t di = PART(part).points_ptr;
    int16_t i;
...
{
    uint16_t si = (PART(part).flags_08 & 0x10) ? 0x3308 : 0x32fc;
    uint16_t di = PART(part).points_ptr;
    int16_t i;
...
void part_setup_1dfb(uint16_t part)
{
    ...
    PART(part).grab_x = 0x38;
    PART(part).grab_y = 0x12;
    PART(part).word_58 = 0x0c;
...
uint16_t part_hook_172c(uint16_t off, uint16_t part)
{
...

    {
         ...
         */
        if (dev_survey_hook(off, PART(part).kind))
            return 0;
...
uint16_t part_hit_0332(uint16_t part)
{
    ...
    int16_t  face = ((int16_t)PART(di).word_8a);

    if ((PART(si).flags_08 & 0x10) != 0) {
        if (face == 1 || face == 3)
            PART(si).direction = 1;
    } else {
        if (face == 0 || face == 4)
...
void part_flip_15fc(uint16_t part)
{
    ...
    if ((int16_t)PART(si).form >= 4)
        PART(si).form = 0;
    else
        PART(si).form = 4;

    PART(si).word_90 = PART(si).form;
...
{
    uint16_t si    = part;
    uint16_t other = PART(si).word_84;
    int16_t  lo    = (int16_t)(PART(other).word_22 + 4);
    int16_t  hi    = (int16_t)(lo + 0x1c);
    int16_t  mid   = (int16_t)(PART(si).word_22
                               + (int16_t)(((int16_t)PART(si).width) >> 1));

    if ((int16_t)((uint16_t)PART(si).word_38) > 0 && mid > lo && mid < hi)
        return 0;
...
void part_settle_2789(uint16_t part)
{
    ...
    form = (int16_t)((int16_t)((int16_t)PART(si).width) / 0x10 - 1);

    PART(si).form = form;
    PART(si).word_90 = form;
...
uint16_t part_step_0405(uint16_t part)
{
    ...
    PART(si).flags_08 |= 0x40;

    if (PART(si).direction == 1) {
        if (((int16_t)PART(si).form) != 2) {
            PART(si).form =
                (int16_t)(((int16_t)PART(si).form) + 1);
            ...
            for (di = PART(si).word_78; di != 0;
                ...
                if ((PART(di).flags_06 & 0x1000) != 0) {
                    ...
                    if (((int16_t)PART(di).kind) == 0x2d) {
                        PART(di).spin = 0;
                        PART(di).direction = 0;
                        PART(di).form = 0;
                    }
                } else if (((int16_t)PART(di).kind) == 0x28) {
                    PART(di).direction = 1;
                    PART(di).spin = 0x14;
                }
            }
               ...
               && ((int16_t)PART(si).form) != 0) {
        PART(si).form =
            (int16_t)(((int16_t)PART(si).form) - 1);
    }

    if (((int16_t)PART(si).form) != ((int16_t)PART(si).word_0e)) {
        part_setup(0x0371, si);

        if (((int16_t)PART(si).word_0e) == 0
            || ((int16_t)PART(si).word_0e) == 2)
            play_sound(0x12);
...
uint16_t part_hit_2c83(uint16_t part)
{
    ...
    if (PART(si).word_96 == 0 && face < 3) {
        PART(si).word_96 = 0x1c;
        PART(si).direction = 0;
...
{
    uint16_t si   = part;
    uint16_t di   = PART(si).word_84;
    int16_t  face = ((int16_t)PART(si).word_8a);
    ...
    if ((int16_t)((uint16_t)PART(si).word_38) > 0
        && (int16_t)(((uint16_t)PART(si).word_88) + 0x800) < 0x1000
        && (int16_t)(((uint16_t)PART(si).pos_y)
                     + PART(si).word_42)
           < (int16_t)(((uint16_t)PART(di).pos_y) + 0x0c))
        PART(di).direction = 1;
...
uint16_t part_step_2d40(uint16_t part)
{
    ...
    if (PART(si).word_96 != 0) {
        ...
        if (PART(si).word_96 == 0) {
            ...
            if ((int16_t)PART(si).form >= 9)
                PART(si).byte_6b = 0x35;

            PART(si).form++;

            if (PART(si).form == 9)
                PART(si).form = 5;
            else if (PART(si).form == 0x0d)
                PART(si).form = 9;
        }
...
uint16_t part_step_332a(uint16_t part)
{
    ...
    if (((uint16_t)PART(di).direction) == 0)
        return 0;
    ...
    if (PART(di).form == 1) {
        ...
        if (si != 0) {
            ...
            PART(si).pos_x =
                (uint16_t)(((uint16_t)PART(di).pos_x) - 0x10);
            PART(si).pos_y = ((uint16_t)PART(di).pos_y);
...
uint16_t part_step_49a1(uint16_t part)
{
    ...
    if (((uint16_t)PART(part).spin) != 0) {
        PART(part).spin--;
        if (((uint16_t)PART(part).spin) != 0)
            PART(part).direction = 1;
    }
    ...
    if (di != 0 && !(PART(di).flags_08 & 0x800)) {
        if (((uint16_t)PART(part).direction) != 0)
            PART(di).direction =
                (PART(part).flags_08 & 0x10) ? 0xffff : 1;
        else
            PART(di).direction = 0;
    }
    ...
    if (PART(part).form != PART(part).word_0e)
...
{
    if (PART(part).spin < 0x14)
        PART(part).form++;

    if (((int16_t)PART(part).form) > 0x16) {
        PART(part).form = 0x0e;
        PART(part).spin++;
    } else if (PART(part).form == 0x0b) {
        PART(part).form = 0;
    }
...
void settle_gear_signal(uint16_t part, int16_t clear)
{
    ...
    PART(part).form =
        (uint16_t)(PART(part).form
                   + ((uint16_t)PART(part).direction));

    if (((int16_t)PART(part).form) == -1)
        PART(part).form = 3;
    else if (((int16_t)PART(part).form) == 4)
        PART(part).form = 0;
...
uint16_t part_step_2592(uint16_t part)
{
    ...
    if (PART(part).direction > 0) {
        if ((int16_t)(((int16_t)PART(part).form) + 1) % 7 == 0)
            PART(part).form -= 6;
        else
            PART(part).form++;
    } else if (PART(part).direction < 0) {
        if (((int16_t)PART(part).form) % 7 == 0)
            PART(part).form += 6;
        else
            PART(part).form--;
    }
...
uint16_t part_step_057e(uint16_t part)
{
    ...
    for (di = PART(part).word_78; di != 0;
        ...
        if (PART(di).flags_06 & 0x1000) {
            ...
            PART(di).vel_x =
                (PART(part).flags_08 & 0x10) ? v : (int16_t)-v;
        } else if (PART(di).kind == 0x0f) {
            break_kind_15(di);
        } else if (PART(di).kind == 6) {
            trigger_kind_6(di);
        }
    }
...
uint16_t part_step_0a5d(uint16_t part)
{
    ...
    if (PART(part).flags_08 & 0x10) {
        PART(si).pos_x =
            (int16_t)(PART(part).pos_x - 0x30);
        PART(si).word_26 =
            (int16_t)(PART(si).pos_x + 0x18);
        PART(si).word_22 = ((int16_t)PART(si).word_26);
        PART(si).vel_x = 0xd000;
    } else {
        PART(si).pos_x =
            (int16_t)(PART(part).pos_x + 0x61);
        PART(si).word_26 =
            (int16_t)(PART(si).pos_x - 0x18);
        PART(si).word_22 = ((int16_t)PART(si).word_26);
        PART(si).vel_x = 0x3000;
    }

    PART(si).pos_y =
        (int16_t)(PART(part).pos_y - 7);
    PART(si).word_28 =
        (int16_t)(PART(si).pos_y + 8);
    PART(si).word_24 = ((int16_t)PART(si).word_28);
    PART(si).word_38 = 0xf000;
...
uint16_t part_step_1a82(uint16_t part)
{
    ...
    if (PART(di).form == PART(di).word_0e)
        play_sound(9);
    ...
    for (si = PART(di).word_78; si != 0;
        ...
        if (PART(si).flags_06 & 0x2000) {
            if (PART(si).kind != 0x28)
                continue;
...
{
    int16_t x = (int16_t)(PART(part).pos_x + dx);
    uint16_t si;

    for (si = PART(part).word_78; si != 0;
         si = PART(si).word_78) {

        int16_t d = (int16_t)(x - PART(si).pos_x);
        int16_t mirrored = (PART(si).flags_08 & 0x10) != 0;
...
uint16_t part_step_420f(uint16_t part)
{
    ...
    if (PART(si).flags_08 & 0x400) {
        PART(si).form =
            (uint16_t)(PART(si).form
                       + ((uint16_t)PART(si).direction));
    } else if (drive_belts(0, si, 0x8000, 0x3e8,
                           PART(si).momentum_lo,
                           PART(si).momentum_hi) != 0) {
        PART(si).flags_08 |= 0x200;
    } else {
    ...
    if (PART(si).word_0e == 0
        || PART(si).word_0e == 2)
        play_sound(0x12);

    place_object_for_draw(si);

    DG16(v02) = (int16_t)(PART(si).pos_x
                          + (((int16_t)PART(si).width) >> 1));

    link_objects_crossing(si, 0x1000,
                          (uint16_t)(0x3542 + 8 * PART(si).form));
    ...
    for (di = PART(si).word_78; di != 0;
        ...
        if (PART(si).direction == -1) {
            if (DG16(v04) < DG16(v02)) {
                PART(di).word_38 = DG16(v06);
                PART(di).vel_x =
                    (int16_t)-(int16_t)(DG16(v06) >> 2);
            } else {
                PART(di).word_38 = (int16_t)-DG16(v06);
                PART(di).vel_x = (int16_t)(DG16(v06) >> 2);
            }
        } else if (PART(si).direction == 1) {
            if (DG16(v04) < DG16(v02)) {
                PART(di).word_38 = (int16_t)-DG16(v06);
        ...
        if (PART(di).word_38 < 0) {
            PART(di).word_24 =
                (int16_t)(PART(di).pos_y - 0x10);
...
tail:
    ...
    if (PART(si).word_0e != 2)
        goto out;
    if (((uint16_t)PART(si).word_10) == 2)
        goto out;
...
uint16_t part_step_38fc(uint16_t part)
{
    ...
    cut_belts(part, (PART(part).flags_08 & 0x10) ? 0x34c2 : 0x34ba);
...
void cut_belts(uint16_t part, uint16_t line)
{
    ...
    for (DGU16(rec) = DG521B.parts_ptr; DGU16(rec) != 0;
         DGU16(rec) = ((uint16_t)PART(DGU16(rec)).link_ptr)) {

        if (PART(DGU16(rec)).kind != 0x0a)
            continue;

        DGU16(belt) = PART(DGU16(rec)).word_66;
        ...
        while (DGU16(prev) != 0 && DGU16(next) != 0) {
            ...
            if (intersect_segments(line, seg, at) == 0) {
                if (DGU16(next) == DGU16(endB)) {
                    ...
                    DGU16(next) = PART(DGU16(next)).link_right;
                }
                continue;
            }
            ...
            DG4E67.state = 0x1000;
            mark_belt_shapes(((uint16_t)BELT(DGU16(belt)).owner_ptr), 3);
            ...
            PART(di).flags_06 |= 0x10;
            PART(di).pos_x =
                (int16_t)(DG16(at) + PART(part).pos_x);
            PART(di).pos_y =
                (int16_t)(DG16((uint16_t)(at + 2))
                          + PART(part).pos_y);

            insert_sorted(DGU16(anchorB), 0x5179);
            PART(DGU16(anchorB)).flags_06 |= 0x10;
            PART(DGU16(anchorB)).pos_y =
                PART(di).pos_y;
            PART(DGU16(anchorB)).pos_x =
                PART(di).pos_x;
            ...
            BELT(DGU16(newbelt)).end_a_ptr = DGU16(anchorB);
            BELT(DGU16(newbelt)).end_b_ptr = DGU16(endB);
            BELT(DGU16(newbelt)).slot_a = 0;
            BELT(DGU16(newbelt)).slot_b =
                BELT(DGU16(belt)).slot_b;

            PART(DGU16(anchorB)).link_right = DGU16(next);
            PART(DGU16(anchorB)).word_66 = DGU16(newbelt);

            if (PART(DGU16(next)).kind == 7) {
                PART(DGU16(next)).word_68 = DGU16(newbelt);
                PART(DGU16(next)).link_left = DGU16(anchorB);
            } else {
            ...
            DGU16((uint16_t)(DGU16(endB) + 0x66
                             + 2 * BELT(DGU16(newbelt)).slot_b)) =
                DGU16(newbelt);

            BELT(DGU16(belt)).end_b_ptr = di;
            BELT(DGU16(belt)).slot_b = 0;
            PART(di).link_right = DGU16(prev);
            PART(di).word_66 = DGU16(belt);

            if (PART(DGU16(prev)).kind == 7)
                PART(DGU16(prev)).link_right = di;
            else
            ...
            PART(DGU16(anchorB)).word_22 =
                PART(DGU16(anchorB)).pos_x;
            PART(DGU16(anchorB)).word_26 =
                PART(DGU16(anchorB)).pos_x;
            ...
            PART(DGU16(anchorB)).word_24 =
                PART(DGU16(anchorB)).pos_y;
            PART(DGU16(anchorB)).word_28 =
                PART(DGU16(anchorB)).pos_y;
...
uint16_t part_step_0ca3(uint16_t part)
{
    ...
    if (PART(si).flags_08 & 0x20) {
        if (PART(si).flags_06 & 2) {
            PART(si).flags_08 &= 0xffdf;
            PART(si).form = 0;
        }
        goto draw;
    }

    if (DG16(still) != 0 || ((int16_t)PART(si).form) >= 2) {
        if (PART(si).form == 1) {
            PART(si).word_96++;
            if (((int16_t)PART(si).word_96) <= 0x0c)
                goto draw;

            DG16(step) = (PART(si).flags_08 & 0x10)
                         ? 0x20 : (int16_t)0xffe0;
            PART(si).word_96 = 0;
            PART(si).pos_x += DG16(step);
            place_object_for_draw(si);

            if (object_overlaps_any(si) != 0) {
                PART(si).pos_x -= (int16_t)(DG16(step) * 2);
                place_object_for_draw(si);
    ...
    for (di = PART(si).word_78; di != 0;
        ...
        if (PART(di).kind == 0x0f) {
            DG16(range) = (((int16_t)PART(di).form) >= 0x0b)
                          ? 0x124 : 0x60;
        } else if (PART(di).kind == 0x2a) {
            DG16(dx) = (int16_t)(PART(di).pos_x
                                 - PART(si).pos_x + 0x10);
            DG16(dy) = (int16_t)(PART(di).pos_y
                                 - PART(si).pos_y);
...
{
    uint16_t di = PART(part).word_84;
    int16_t cx = PART(di).direction;
    const int16_t v = 0x1000;

    if (PART(part).word_8a == 0) {
        if (cx > 0) {
            PART(part).vel_x += v;
            if (PART(part).vel_x > v)
                PART(part).vel_x = v;
        } else if (cx < 0) {
            PART(part).vel_x -= v;
            if (PART(part).vel_x < v)
                PART(part).vel_x = (int16_t)-v;
        }
    } else if (PART(part).word_8a == 2) {
        if (cx < 0) {
            PART(part).vel_x += v;
            if (PART(part).vel_x > v)
                PART(part).vel_x = v;
...
uint16_t part_hit_3fe8(uint16_t part)
{
    ...
    if (PART(si).flags_08 & 0x200) {
        answer = 1;
        goto out;
    }

    DGU16(face) = PART(di).word_8a;

    DG16(plain) = (DGU16(face) == 0 || DGU16(face) == 2 || DGU16(face) == 6)
                  ? 0 : 1;

    if (DGU16(face) == 0) {
        DG16(along) = (int16_t)(PART(di).pos_x
                                + (((int16_t)PART(di).width) >> 1)
                                - PART(si).pos_x);
...
{
    uint16_t di = PART(part).word_84;
    int16_t face = ((int16_t)PART(part).word_8a);

    if (PART(di).flags_08 & 0x10) {
        if (face == 1 || face == 2 || face == 4 || face == 5)
            PART(di).direction = 1;
        else if (face == 7 && PART(part).kind == 4)
            PART(part).direction = 1;
    } else {
        if (face == 0 || face == 1 || face == 5 || face == 6)
            PART(di).direction = 1;
        else if (face == 3 && PART(part).kind == 4)
            PART(part).direction = 1;
    }
...
uint16_t part_hit_14d3(uint16_t part)
{
    uint16_t di = part;
    uint16_t si = PART(di).word_84;
    uint16_t turned = (uint16_t)(((uint16_t)PART(di).word_88) + 0x4000);
    ...
    PART(si).direction =
        (PART(si).form != PART(si).word_90) ? 1 : 0;

    if (PART(di).kind == 0x14)
        PART(di).vel_x--;
...
{
    trigger_kind_6(PART(part).word_84);
    return 1;
}
...
uint16_t part_step_13c9(uint16_t part)
{
    ...
    if (di != 0 && !(PART(di).flags_08 & 0x800)) {
        if (((uint16_t)PART(si).direction) == 0)
            ...
            PART(di).direction =
                (PART(si).flags_08 & 0x10) ? 1 : 0xffff;
    }
...
uint16_t part_hit_3ebf(uint16_t part)
{
    ...
    if (PART(si).word_8a != 0)
        return 1;

    apart = (int16_t)((PART(si).pos_x
                       + (int16_t)(((int16_t)PART(si).width) >> 1))
                      - (PART(di).pos_x
                         + (int16_t)(PART(di).width >> 1)));
    ...
    if ((v < 0 ? (int16_t)-v : v) >= 0x400)
        PART(si).vel_x = (int16_t)(v >> 1);

    PART(si).word_84 = 0;
    PART(si).flags_06 &= 0xfffe;
...
uint16_t part_step_27e2(uint16_t part)
{
    ...
    if (((int16_t)PART(si).form) > 7) {
        ...
        if (PART(si).form == 8) {
            PART(si).form = 0;
            PART(si).word_96++;
        } else if (((int16_t)PART(si).form) == -1) {
            PART(si).form = 7;
            PART(si).word_96++;
        }

        if (PART(si).word_96 == 6) {
            play_sound(3);
            PART(si).form = 8;
        }
    }
...
uint16_t part_step_22ae(uint16_t part)
{
    ...
    PART(si).word_28 =
        (int16_t)(PART(di).pos_y + 3);
    PART(si).word_24 = ((int16_t)PART(si).word_28);
    PART(si).pos_y = ((int16_t)PART(si).word_28);
...
uint16_t part_step_08f1(uint16_t part)
{
    ...
    if (((uint16_t)PART(si).vel_x) == 0x3000
        || ((uint16_t)PART(si).vel_x) == 0xd000)
        return 0;

    PART(si).point_count = 0;
    PART(si).form = 1;
    place_object_for_draw(si);
    play_sound(0x0b);
