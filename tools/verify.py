...
def count_transcribed():
    ...
    **This is ours, not a transcription.** It exists because the caption used to
    ...
    with the original - it lacked the "ours" branch and the file-header
    exclusion, and arrived at the same total by two errors cancelling.
    """
    ...
    for path in sorted(glob.glob(os.path.join(root, "reconstruct", "*.c"))):
        transcribed, _ours, _stubs, _bare = provenance.check(path)
