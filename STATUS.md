...
| `part_flip_options` | 0x04748 | - | **transcribed, never called** on these screens |
| `find_part_from` | 0x04500 | - | **transcribed, never called** on these screens |
| `part_under_pointer` | 0x042a2 | - | **transcribed, never called** on these screens |
| `bin_scroll_back` | 0x10cc8 | - | **transcribed, never called** on these screens |
| `bin_scroll_forward` | 0x10d37 | - | **transcribed, never called** on these screens |
| `bin_scroll_end` | 0x058bb | - | **transcribed, never called** on these screens |
| `vm_set_line_compare` | 0x08f27 | - | **transcribed, never called** on these screens |
...
| `sound_service` | 0x27ace | - | **transcribed, never called** on these screens |
| `drop_unless_polled` | 0x27b52 | - | **transcribed, never called** on these screens |
| `poll_sequences` | 0x27b7e | - | **transcribed, never called** on these screens |
| `sequencer_tick` | 0x26f2a | 0 (missed 1) | **not verified** |
| `remove_sequence` | 0x26e7b | 0 (missed 1) | **not verified** |
...
| `start_sound` | 0x29c3b | 0 | **differs** |
| `setup_sound_device` | 0x28655 | 0 | **differs** |
...
| `insert_by_key` | 0x28ddb | 0, 1, 4 | agreed |
...
| `set_sound_callback` | 0x2928c | 0 | agreed |
| `set_master_level_ok` | 0x296a1 | 0 | agreed |
| `alloc_voice_records` | 0x28800 | - | **transcribed, never called** on these screens |
| `stop_music_or_effect` | 0x083ea | - | **transcribed, never called** on these screens |
...
| `timer_install` | 0x206c1 | 0 (missed 1) | **not verified** |
| `timer_add_callback` | 0x20654 | 0 (missed 1) | **not verified** |
...
| `long_multiply_2` | 0x0bcf6 | - | **transcribed, never called** on these screens |
| `long_multiply` | 0x0c16e | 0, 1, 4 | agreed |
| `long_shift_right` | 0x0be62 | - | **transcribed, never called** on these screens |
| `long_divide` | 0x0bd93 | - | **transcribed, never called** on these screens |
| `score_code_to_score` | 0x02900 | - | **transcribed, never called** on these screens |
| `score_to_code` | 0x02809 | - | **transcribed, never called** on these screens |
| `read_password_line` | 0x12b60 | - | **transcribed, never called** on these screens |
| `show_level_complete` | 0x158c5 | - | **transcribed, never called** on these screens |
...
| `flush_pending_volumes` | 0x27a86 | - | **transcribed, never called** on these screens |
...
| `asb_dsp_write` | SX.OVL ASB:0x0377 | - | **transcribed, never called** on these screens |
| `asb_dsp_write_try` | SX.OVL ASB:0x070e | - | **transcribed, never called** on these screens |
| `asb_probe_reset` | SX.OVL ASB:0x06c0 | - | **transcribed, never called** on these screens |
| `asb_probe_identify` | SX.OVL ASB:0x06eb | - | **transcribed, never called** on these screens |
| `asb_probe_version` | SX.OVL ASB:0x075d | - | **transcribed, never called** on these screens |
| `asb_dma_program` | SX.OVL ASB:0x08ec | - | **transcribed, never called** on these screens |
| `asb_set_rate` | SX.OVL ASB:0x031b | - | **transcribed, never called** on these screens |
...
| `heap_ring_unlink` | 0x0c95a | 0, 1 (missed 4) | **not verified** |
| `heap_ring_insert` | 0x0c976 | 0, 1 (missed 4) | **not verified** |
| `heap_free_top` | 0x0c8e7 | 0, 1, 4 | agreed |
| `heap_free_middle` | 0x0c921 | 0, 1 (missed 4) | **not verified** |
...
| `heap_check` | 0x0cb45 | - | **transcribed, never called** on these screens |
| `draw_bitmap` | 0x25300 | 0 (missed 1, 4) | **not verified** |
| `load_screen` | 0x253e7 | - | **transcribed, never called** on these screens |
| `load_screen_plain` | 0x23b29 | - | **transcribed, never called** on these screens |
| `free_bitmaps` | 0x23a3c | - | **transcribed, never called** on these screens |
| `poly_walk` | 0x1f562 | - | **transcribed, never called** on these screens |
| `poly_edge_vertical` | 0x1f265 | - | **transcribed, never called** on these screens |
...
| `poly_edge_shallow_right` | 0x1f3e6 | - | **transcribed, never called** on these screens |
| `poly_edge_shallow_left` | 0x1f4a1 | - | **transcribed, never called** on these screens |
| `clip_polygon` | 0x20c07 | - | **transcribed, never called** on these screens |
| `poly_outline` | 0x1f219 | - | **transcribed, never called** on these screens |
| `restore_write_mode` | 0x1e94c | - | **transcribed, never called** on these screens |
| `seg172c_nothing` | 0x172bc | - | **transcribed, never called** on these screens |
| `free_bitmaps_thunk` | 0x252d0 | - | **transcribed, never called** on these screens |
| `game_fread_byte` | 0x11db4 | - | **transcribed, never called** on these screens |
| `select_cursor` | 0x0467d | 0 (missed 1, 4) | **not verified** |
| `set_cursor` | 0x0aa14 | 0 | agreed |
| `draw_cursor` | 0x0ab1f | 0 | agreed |
| `redraw_cursor` | 0x0acc3 | 0 | agreed |
| `read_list` | 0x1221b | - | **transcribed, never called** on these screens |
| `read_level` | 0x12269 | - | **transcribed, never called** on these screens |
| `load_animation` | 0x12915 | - | **transcribed, never called** on these screens |
| `alloc_part_table` | 0x11d66 | - | **transcribed, never called** on these screens |
| `draw_frame_corners` | 0x0ee6e | - | **transcribed, never called** on these screens |
| `load_all_parts` | 0x0f7b6 | - | **transcribed, never called** on these screens |
| `load_part_bitmap` | 0x0f7f4 | - | **transcribed, never called** on these screens |
| `build_part_list` | 0x1405b | - | **transcribed, never called** on these screens |
...
| `regions_handle_pointer` | 0x08546 | - | **transcribed, never called** on these screens |
...
*1221 routines transcribed. **This run asked about 610 of them** and 186 agreed; the other 651 were not asked, and are **unchecked, not disproved**. Written by `tools/verify.py --all`, not by hand - one run of the original captures every call. This one was **with no input**, in 88 seconds; "never called" means that run did not reach it. Specs added after a sweep starts are not in the table it writes: compare the row count against `verify.py --list`.*
<!-- VERIFY:END -->
...
### `start_sound` and `setup_sound_device` differ, and did before this work

Both are the sound device's setup, both are seen once, and both diverge at the
same place: the original's fifth port write is `0x246 = 01` and the port's is
`0x22c = e1`. The harness then reports **RAN OUT of primed allocations** - the
original made six during the call and the port asked for two more - and says
so itself: *nothing after the first is evidence about the transcription.*

The Sep-6 table above recorded both as **agreed**, which is why they are worth
a note rather than a row. Measured on 2026-09-09 at `db0fe9a`, the commit
before three days of frame work, they differ there too. So this is older than
