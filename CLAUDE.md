...
| `tools/check_image_data.py` | **proves each placed object is the image's**: at its address in `libtim.so`, holding the relocated image's bytes (or zero, for `DGROUP_BSS`). Runs in `make test` |
