...
| `tools/check_part_kinds.py` | **proves the transcribed kind table is the image's.** The 58 records at DGROUP 0x0ea6 are C data in `dgroup.c` and `load_part_kinds` puts them in DGROUP; this loads the program as the port does and compares, the relocated hook segments included. Runs in `make test` |
