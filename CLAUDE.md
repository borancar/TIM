...
| `tools/check_machines.py` | **proves the hybrid and the port run the same solved machine, flip for flip.** The same `solutions/S<NN>.TIM` files, loaded the same way on both sides into a staged game directory, hashed per guest page flip and compared from the flip each side says it started the machine - the one signal the game itself gives, and the only alignment. **The port is run twice and that control is the verdict**: a flip its two runs disagree on is excluded and counted, because holding the hybrid to one of two answers would be a coin toss. Measured 2026-09-18: 29 of 29 levels, 685 flips each, byte for byte, no unstable flip |
...
| `tools/check_dg_near.py` | **refuses a `dg_near` that is not a store into a near-pointer field.** Turning a pointer back into its DGROUP offset belongs where the value is filed into guest memory, so over a tree-sitter parse a `dg_near(...)` call must be the whole right-hand side of an assignment to a struct field named `_ptr` and declared `dg_near_t`: never compared (the stored field is turned into a pointer instead), cast, passed, returned or held in a local. A typedef of `dg_near_t` (`bmp_ptr_t`) counts; it runs in `make test`, ahead of the solutions step |
| `tools/check_handles.py` | **refuses a typed handle used as a boolean.** `PART_PTR(0)` is DGROUP:0, never NULL, so `if (p)`, `p ? :`, `!p` and `p &&` on a `struct part *`, `struct belt *`, `struct rope *`, `struct rect_list_entry *`, `struct game_file *` or `struct bitmap *` are always true where the original tested an offset against 0 - and C accepts them. Structural, because the fault's condition is one no captured run reaches, and **over the parse tree**: `if (p)`, `!p`, `p ? :` and the two sides of a `&&` are node types, where the regex it replaced needed five lookbehinds and still called a `p == NULL` four lines after its own `heap_calloc_far` a fault. It says out loud when a file has a region it could not parse; it runs in `make test`, ahead of the solutions step |
...
| `tools/cparse.py` | **the port's C, parsed** - the one door to tree-sitter for every tool that reads the sources. Four things here are macros the C grammar has no rule for: `DGROUP_AT` between a declarator and its `=`, `DG_ASSERT_AT` and `__builtin_offsetof` taking a *type* as an argument, and `far`, which `tim.h` defines as nothing. Each makes tree-sitter abandon the construct and read what follows as loose expressions - a plain parse of this tree is **7,188 ERROR nodes**, 6,112 of them in `dgroup.c`, and inside an ERROR subtree the node types are wrong, so a rule walking it is reading soup and cannot tell. Expanding the four, space for space so every offset is the file's own, leaves **six**: two preprocessor conditionals in the middle of a function, which no expansion can help |
